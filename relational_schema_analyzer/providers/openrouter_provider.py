...
# Best-effort scrubbing of secret-like substrings from upstream error bodies
# before embedding them in an LLMError (the common leak is the bearer token).
_SECRET_PATTERNS = (
...
class _NoRedirectHandler(urllib.request.HTTPRedirectHandler):
    """Block redirects so a typo'd base_url can't shuttle credentials elsewhere."""

    def redirect_request(self, req, fp, code, msg, headers, newurl):  # type: ignore[override]  # pragma: no cover
        raise urllib.error.HTTPError(
            req.full_url, code, f"redirect blocked ({code} -> {newurl})", headers, fp
        )
...
class OpenRouterProvider:
    ...
    def generate(
        ...
        try:
            ...
            raise LLMError(
                f"OpenRouter request failed (HTTP {e.code}): {scrubbed}", code="PROVIDER_ERROR"
            ) from e
        except Exception as e:  # pragma: no cover
            raise LLMError(f"OpenRouter request failed: {e}", code="PROVIDER_ERROR") from e

        try:
            data = json.loads(raw)
            text = data["choices"][0]["message"]["content"] or ""
        except Exception as e:  # pragma: no cover
            raise LLMError("OpenRouter response parse failed", code="PROVIDER_ERROR") from e
